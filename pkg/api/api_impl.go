...
func (ctx *internalContext) rebuild() rebuildState {
	...
	if args.options.HMR {
		build.state = streamBuildImpl(args)
	} else {
...
func validateBuildOptions(
	...
	for _, ep := range buildOpts.EntryPointsAdvanced {
		entryPoints = append(entryPoints, bundler.EntryPoint{InputPath: ep.InputPath, OutputPath: ep.OutputPath, Name: ep.Name})
	}
	entryPointCount := len(entryPoints)
	if entryPointCount == 1 {
		if entryPoints[0].Name == "" {
			entryPoints[0].Name = "main"
		}
	} else if entryPointCount > 1 {
		entryNameMap := make(map[string]bool, entryPointCount)
		for _, e := range entryPoints {
			if !entryNameMap[e.Name] {
				entryNameMap[e.Name] = true
			} else {
				log.AddError(nil, logger.Range{}, fmt.Sprintf("Duplicate entry name: %s", e.Name))
			}
		}
	}
...
func rebuildImpl(args rebuildArgs, oldSummary buildSummary) rebuildState {
	...
	runOnEndCallbacks(log, timer, args.onEndCallbacks, &result)
...
func incrementalBuildImpl(args incrementalBuildArgs) rebuildState {
	...
	compileResult := bundle.Compile(log, timer, result.MangleCache)

	result.Metafile = compileResult.MetaJSON
	...
	// Populate the results to return
	result.OutputFiles = make([]OutputFile, len(compileResult.OutputFiles))
	result.Chunks = compileResult.Chunks
	result.EntryPoints = args.entryPoints

	for i, item := range compileResult.OutputFiles {
		if args.options.WriteToStdout {
			item.AbsPath = "<stdout>"
		}
		result.OutputFiles[i] = OutputFile{
	...
	return rebuildState{
		watchData: compileResult.WatchData,
		result:    result,
		options:   args.options,
		summary:   summarizeOutputFiles(result.OutputFiles),
	}
}

func streamBuildImpl(args rebuildArgs) rebuildState {
	log := logger.NewStderrLog(args.logOptions)
	var result BuildResult
...
func runOnEndCallbacks(log logger.Log, timer *helpers.Timer, callbacks []onEndCallback, result *BuildResult) {
	// Run any registered "OnEnd" callbacks now
	timer.Begin("On-end callbacks")
	for _, onEnd := range callbacks {
