...
func packCode(initialChunkId string, initialModuleId string, hash string) string {
	...
	code := `/******/ (function(modules) { // webpackBootstrap
		...
		/******/  __webpack_require__("{{initialModuleId}}")
		/******/
	...
	return m.ReplaceAllStringFunc(code, func(s string) string {
		if s == "{{initialChunkId}}" {
			return initialChunkId
		}
		if s == "{{initialModuleId}}" {
...
func GeneratePackSource(index uint32, initialChunkId string, initialModuleId string, hash string) logger.Source {
	return logger.Source{
		Index:          index,
		KeyPath:        logger.Path{Text: "<pack_runtime>"},
		PrettyPath:     "<pack_runtime>",
		IdentifierName: "pack_runtime",
		Contents:       packCode(initialChunkId, initialModuleId, hash),
	}
}
